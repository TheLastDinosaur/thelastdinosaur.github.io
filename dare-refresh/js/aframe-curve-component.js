...
AFRAME.registerComponent('curve', {
    ...
    schema: {
        type: {
            type: 'string',
            default: 'CatmullRom',
            oneOf: ['CatmullRom', 'CubicBezier', 'QuadraticBezier', 'Line', 'Custom']
        },
        closed: {
            type: 'boolean',
            default: false
    ...
    update: function (oldData) {
        ...
        if (!AFRAME.utils.deepEqual(pointsArray, this.pathPoints) || (oldData !== 'CustomEvent' && !AFRAME.utils.deepEqual(this.data, oldData))) {
            ...
            if (this.data.type && this.data.type !== "Custom") {
                this.createStandardCurve();
            } else {
                this.curve = new THREE.CurvePath();

                // Main loop - through all points
                for (var index = 0; index < this.pathPoints.length - 1;) {
                    const type = this.points[index].getAttribute("type");
                    if (!type || type === "Line") {
                        const p1 = this.pathPoints[index]
                        const p2 = this.pathPoints[index + 1]
                        index += 1;

                        this.curve.add(new THREE.LineCurve3(p1, p2))
                    } else if (type === "CubicBezier") {
                        const p1 = this.pathPoints[index]
                        const p2 = this.pathPoints[index + 1]
                        const p3 = this.pathPoints[index + 2]
                        const p4 = this.pathPoints[index + 3]
                        index += 3;

                        this.curve = new THREE.CubicBezierCurve3(p1, p2, p3, p4);
                        ...
                        const points = [this.pathPoints[index]]
                        index += 1

                        // loop through all untyped
                        while (index < this.pathPoints.length) {
                            points.push(this.pathPoints[index])

                            const nexttype = this.points[index].getAttribute("type");
                            if (nexttype) break;
                            if (index >= this.points.length -1) break;

                            index += 1
                        }

                        if (type === "Spline")
                            this.curve.add(new THREE.SplineCurve3(points))
                        else 
                            this.curve.add(new THREE.CatmullRomCurve3(points))
                    }
                }
            }
